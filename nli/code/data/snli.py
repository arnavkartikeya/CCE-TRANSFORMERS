...
def load_for_analysis(
    ...
    **kwargs
    ...
    if model_type == "bert":
        ...
        encoder = AutoModel.from_pretrained(ckpt["encoder_name"])
        tokenizer = AutoTokenizer.from_pretrained(ckpt["encoder_name"])
        
        # Create BertEntailmentClassifier
        model = models.BertEntailmentClassifier(encoder_name=ckpt["encoder_name"], vocab=vocab)
        
        # Load the state dict
        model.mlp.load_state_dict(ckpt["mlp_state_dict"])
        model.bn.load_state_dict(ckpt["bn_state_dict"])
        model.dropout.p = ckpt["dropout_p"]
        
        # # Set the encoder and tokenizer
