...
def search_feats(acts, states, feats, weights, dataset):
    print('Entered search_feats function')

    # Check if settings.RESULT is defined and is a valid path
    if not hasattr(settings, 'RESULT'):
        raise AttributeError("settings.RESULT is not defined")
    if not os.path.isdir(os.path.dirname(settings.RESULT)):
        raise ValueError(f"Invalid RESULT directory: {settings.RESULT}")

    print(f"RESULT directory: {settings.RESULT}")

    rfile = os.path.join(settings.RESULT, "result.csv")
    print(f"Checking for cached file: {rfile}")
    ...
    print("No cached file found or error loading it. Proceeding with computation.")
    ...
    n_done = 0
    print(f"Starting processing with {len(units)} units")
    
    try:
        for unit in tqdm(units, desc="Processing Units"):
            print(f"Processing unit {unit}")  # Add this line for debugging
            res = compute_best_sentence_iou((unit,))
            ...
            if n_done % settings.SAVE_EVERY == 0:
                ...
                print(f"Saved progress: {n_done}/{len(units)} units processed")

    except Exception as e:
        print(f"Error occurred during processing: {e}")
        traceback.print_exc()

    print(f"Processing completed. Total units processed: {n_done}")
    pd.DataFrame(records).to_csv(rfile, index=False)
    print(f"Final results saved to {rfile}")
    return records
