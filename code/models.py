...
class SentimentTransformerClassifier(nn.Module):
    ...
    def get_final_reprs(self, text, length=None):
        """
...
class EntailmentClassifier(nn.Module):
    def __init__(self, encoder):
        ...
        self.encoder = encoder
        self.encoder_dim = encoder.output_dim
        self.mlp_input_dim = self.encoder_dim * 4  # Concatenate representations
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(self.mlp_input_dim//2, 3),
        )

        self.output_dim = 3

    def forward(self, s1_input_ids, s1_attention_mask, s2_input_ids, s2_attention_mask):
        s1enc = self.encoder(s1_input_ids, s1_attention_mask)
        s2enc = self.encoder(s2_input_ids, s2_attention_mask)
        elem_prod = s1enc * s2enc
        diffs = s1enc - s2enc
        mlp_input = torch.cat((s1enc, s2enc, diffs, elem_prod), dim=1)
        mlp_input = self.bn(mlp_input)
        mlp_input = self.dropout(mlp_input)
        preds = self.mlp(mlp_input)
        return preds

    def get_final_reprs(self, s1_input_ids, s1_attention_mask, s2_input_ids, s2_attention_mask):
        """
        Get features right up to final decision
        """
        s1enc = self.encoder(s1_input_ids, s1_attention_mask)
