...
def run(split, epoch, model, optimizer, criterion, dataloaders, args):
    ...
    tokenizer = model.encoder.tokenizer  # Access the tokenizer from the model
    ...
    for (s1, s1len, s2, s2len, targets) in ranger:
        ...
        with ctx():
            # Tokenize the raw text sequences and move them to the correct device
            s1_inputs = tokenizer(s1, return_tensors='pt', padding=True, truncation=True, max_length=model.encoder.max_length).to(device)
            s2_inputs = tokenizer(s2, return_tensors='pt', padding=True, truncation=True, max_length=model.encoder.max_length).to(device)

            # Forward pass
            logits = model(s1_inputs['input_ids'], s1_inputs['attention_mask'], s2_inputs['input_ids'], s2_inputs['attention_mask'])
