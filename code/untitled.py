...
class BERTTextEncoder(nn.Module):
    ...
    def forward(self, s, slen=None):
        attention_mask = (s != 1).long()
        outputs = self.bert(input_ids=s, attention_mask=attention_mask)
        cls_output = outputs.last_hidden_state[:, 0, :]
        return cls_output
    
    def get_states(self, s, slen=None):
        attention_mask = (s != 1).long()
        outputs = self.bert(input_ids=s, attention_mask=attention_mask)
        return outputs.last_hidden_state

    def get_last_cell_state(self, s, slen=None):
        raise NotImplementedError("BERT does not use LSTM cell states.")
